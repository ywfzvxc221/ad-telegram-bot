...
config = {
    "daily_reward": 0.01,
    "task_reward": 0.02,
    "welcome_message": "أهلاً وسهلاً بك في بوت الربح المجاني! اكسب المال عبر المهام اليومية ودعوة أصدقائك!",
...
def main_menu():
    ...
    markup.row(InlineKeyboardButton("/start - الرجوع للقائمة الرئيسية", callback_data="start"))
...
def handle_admin_input(message):
    if message.text.startswith("/setreward "):
        try:
            ...
            bot.reply_to(message, f"تم تعيين المكافأة اليومية إلى {value}$")
        except:
            bot.reply_to(message, "يرجى إرسال رقم صحيح. مثال: /setreward 0.02")
